...
export function StudentExamsScreen({ navigation }: any) {
  const { student } = useAuthStore();
  const { exams, fetchExams } = useDataStore();
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['bottom']}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.textMuted }]}>Upcoming Schedules</Text>
          {upcomingExams.length === 0 ? (
            ...
            upcomingExams.map((item: any) => (
              <View key={item.id} style={[styles.examCard, { backgroundColor: colors.surface, borderColor: colors.borderLight }, shadows.xs]}>
                ...
                <View style={[styles.examDetails, { borderTopColor: colors.borderLight }]}>
                  <View style={styles.detailItem}>
                    ...
                    <Text style={[styles.detailText, { color: colors.text }]}>{item.exam_date ? new Date(item.exam_date).toLocaleDateString() : 'TBD'}</Text>
                  </View>
                  <View style={styles.detailItem}>
                    <Ionicons name="pin-outline" size={16} color={colors.textMuted} style={styles.detailIcon} />
                    <Text style={[styles.detailText, { color: colors.text }]}>{item.room_no || 'Room 201'}</Text>
                  </View>
                </View>
              </View>
            ))
