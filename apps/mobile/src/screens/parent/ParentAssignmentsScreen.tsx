...
export function ParentAssignmentsScreen({ navigation }: any) {
  const { selectedChild } = useAuthStore();
  const { parentAssignments, fetchParentAssignments } = useDataStore();
  ...
  const loadData = async () => {
    if (selectedChild?.id) {
      await fetchParentAssignments(selectedChild.id);
    }
  };
  ...
  const filtered = (parentAssignments || []).filter((a) => {
