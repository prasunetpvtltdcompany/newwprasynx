...
export function ParentMessagesScreen({ navigation }: any) {
  const { user } = useAuthStore();
  const { parentMessages, fetchParentMessages } = useDataStore();
  const isDark = useThemeStore((s) => s.isDark);
  const colors = isDark ? darkColors : lightColors;

  useEffect(() => {
    if (user?.id) {
      fetchParentMessages(user.id);
    }
  }, [user?.id]);

  const chats = parentMessages || [];
                ...
                <View style={styles.chatHeader}>
                  <View style={[styles.avatarCircle, { backgroundColor: colors.primaryLight }]}>
                    <Text style={[styles.avatarText, { color: colors.primary }]}>
                      {item.other_user_name ? item.other_user_name[0].toUpperCase() : 'T'}
                    </Text>
                  </View>
                  <View style={styles.chatInfo}>
                    <Text style={[styles.chatName, { color: colors.text }]}>{item.other_user_name || 'Instructor'}</Text>
