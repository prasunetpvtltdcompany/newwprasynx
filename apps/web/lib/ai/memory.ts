...
export async function getMemory(userId: string): Promise<string> {
  ...
  const memoryString = (data as Array<{ type: string; key: string; value: string }>).map((m) =>
...
export async function getConversationHistory(
  ...
  if (!data) return [];
  return (data as Array<{ role: string; content: string }>).map((m) => ({ role: m.role, content: m.content }));
}
