...
const quickActions = [
  ...
  { label: 'Send Notice', icon: Bell, intent: 'notification', color: 'text-orange-500' },
];
...
export default function VoiceAITab() {
  const [messages, setMessages] = useState<Message[]>([{
    role: 'assistant',
    content: "Hello! I'm Prasunet AI Voice Assistant. I can help with complaints, appointments, support tickets, student information, job postings, and notifications. How can I assist you today?",
    timestamp: new Date(),
  }]);
  ...
  const startCall = useCallback(async () => {
    try {
      const res = await fetch(`${VOICE_API}/incoming-call`, {
        ...
        body: JSON.stringify({ from: 'management-portal', callerName: 'Management', callerRole: 'management' }),
      });
  ...
  const handleQuickAction = async (intent: string) => {
    const labels: Record<string, string> = {
      ...
      job: 'post a job', notification: 'send a notification',
    };
    ...
    <div className="flex flex-col h-[calc(100vh-12rem)]">
      <div className="flex items-center gap-2 p-3 border-b border-gray-200 bg-white">
        ...
        <div className="flex-1">
          ...
          <div className="text-[10px] text-gray-400">School Management Portal</div>
        </div>
        <div className="flex items-center gap-1">
          {isSpeaking && <button onClick={() => { stopSpeaking(); setIsSpeakingState(false); }} className="p-1.5 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100" title="Stop speaking"><Speaker size={14} /></button>}
          {callActive && <button onClick={endCall} className="p-1.5 rounded-lg bg-red-50 text-red-600 hover:bg-red-100" title="End call"><PhoneOff size={14} /></button>}
      ...
      {activeView === 'chat' ? (
        ...
        <div className="flex-1 p-4">
          <div className="text-center text-gray-400 py-12">
            ...
            <p className="text-xs mt-1">Your call transcripts will appear here.</p>
          </div>
        </div>
      )}
    </div>
