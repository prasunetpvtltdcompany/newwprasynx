  ...
  useEffect(() => {
    const initSocket = async () => {
      ...
      const s = io(wsUrl, {
        ...
        auth: { token: typeof window !== 'undefined' ? localStorage.getItem('staffSession') ? JSON.parse(localStorage.getItem('staffSession') || '{}').token : '' : '' },
        autoConnect: true,
        reconnection: true,
      });
        ...
        <AnimatePresence>
          {showLangPicker && (
            <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }}
              className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 bg-white border border-gray-200 rounded-xl shadow-lg z-10 p-1 whitespace-nowrap"
            >
              {languages.map(l => (
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed top-0 right-0 bottom-0 z-50 w-full sm:w-[450px] max-w-full bg-white shadow-2xl flex flex-col"
      >
          ...
          <div className="flex items-center gap-1">
            <div className="relative">
              ...
              <AnimatePresence>
                {showLangPicker && (
                  <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }}
                    className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-10 p-1 whitespace-nowrap"
                  >
                    {languages.map(l => (
                      <button key={l.code} onClick={() => { setLanguage(l.code); setShowLangPicker(false); }}
                        className={`w-full text-left px-2.5 py-1.5 rounded-lg text-[10px] ${language === l.code ? 'bg-purple-50 text-purple-700 font-semibold' : 'text-gray-600 hover:bg-gray-50'}`}
