...
const quickActions = [
  ...
  { label: 'Talk to Teacher', icon: CalendarDays, intent: 'appointment', color: 'text-blue-500' },
  { label: 'Support Ticket', icon: TicketCheck, intent: 'ticket', color: 'text-purple-500' },
  { label: 'My Child Info', icon: GraduationCap, intent: 'info', color: 'text-indigo-500' },
  { label: 'Find Part-Time Job', icon: Briefcase, intent: 'job', color: 'text-green-500' },
  { label: 'Health Concern', icon: Heart, intent: 'notification', color: 'text-pink-500' },
];
...
export default function VoiceAITab() {
  const [messages, setMessages] = useState<Message[]>([{
    role: 'assistant',
    content: "Hello! I'm Prasunet AI Voice Assistant. I can help you with complaints, teacher meetings, support tickets, student information, part-time jobs, and health concerns. How can I assist you today?",
    timestamp: new Date(),
  }]);
  ...
  const startCall = useCallback(async () => {
    try {
      const res = await fetch(`${VOICE_API}/incoming-call`, {
        ...
        body: JSON.stringify({ from: 'parents-portal', callerName: 'Parent', callerRole: 'parents' }),
      });
  ...
  const handleQuickAction = async (intent: string) => {
    const labels: Record<string, string> = {
      complaint: 'register a complaint', appointment: 'schedule a meeting with a teacher',
      ticket: 'create a support ticket', info: 'get information about my child',
      job: 'find a part-time job', notification: 'report a health concern',
    };
    ...
    <div className="flex flex-col h-[calc(100vh-12rem)]">
      <div className="flex items-center gap-2 p-3 border-b border-gray-200 bg-white">
        ...
        <div className="flex-1">
          ...
          <div className="text-[10px] text-gray-400">Parents Portal</div>
        </div>
