...
const roleSuggestions: Record<string, string[]> = {
  parent: ['Fee Status', 'Homework', 'Book PTM', 'Exam Results', 'Transport Tracking'],
...
function getRoleTasks(role: string, setActiveTab: (tab: Tab) => void, handleSend: (msg: string) => void): TaskItem[] {
  const tasks: Record<string, TaskItem[]> = {
    parent: [
      { icon: Users, label: 'View Child Performance', desc: 'Grades & academic progress', color: '#6D4CFF', action: () => { setActiveTab('chat'); handleSend('Show child performance'); } },
...
function getRoleHistory(role: string): { icon: any; label: string; desc: string; color: string; count: number }[] {
  const history: Record<string, any[]> = {
    parent: [
      { icon: Users, label: 'Academic Inquiries', desc: 'Child performance checks', color: '#6D4CFF', count: 12 },
...
function getRoleRecentActivity(role: string): { action: string; time: string }[] {
  const activity: Record<string, { action: string; time: string }[]> = {
    parent: [
      { action: 'Performance review for Rohan', time: '2h ago' },
  ...
  useEffect(() => {
    const initSocket = async () => {
      ...
      const s = io(wsUrl, {
        ...
        auth: { token: typeof window !== 'undefined' ? localStorage.getItem('parentSession') ? JSON.parse(localStorage.getItem('parentSession') || '{}').token : '' : '' },
        autoConnect: true,
        reconnection: true,
      });
